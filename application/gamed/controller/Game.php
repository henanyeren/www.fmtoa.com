...
class Game extends Common {
    ...
    public function games(){
        ...
        $data=db('ad_game_name')->where($myname)->find();
        if(cookie('new')==1) {
            if($data && $data['pid']!=0){
                $myboss=db('ad_game_boss')->where('id',$data['pid'])->find();
                $this->assign([
            ...
            if($data){
                db('ad_game_name')->update(['id'=>$data['id'],'pid'=>$id]);
                db('ad_game_boss')->update(['id'=>$id,'pid'=>$data['id']]);
            }else{
                $myname['pid']=$id;
                $myname['add_time']=time();
                $pid=db('ad_game_name')->insertGetId($myname);
                db('ad_game_boss')->update(['id'=>$id,'pid'=>$pid]);
            }
            cookie('id',$id);
            return view('mygame');
        }else{
            $myuid = db('ad_game_boss')->where('id', cookie('id'))->find();
            $where = [
                'id' => cookie('id'),
                'uid' => $myuid['uid'] + 1,
            ];
    ...
    public function mygame(){
            $this->redirect('http://www.fmtoa.com/gamed/index.html?id='.cookie('id'));
    }
    ...
    public function boss(){
        ...
        if($id){
            ...
            $myuid = db('ad_game_boss')->where('id',$id)->find();
            //游戏结束跳转地址 及参数
            $myuid['url']="http://www.fmtoa.com/gamed/game/over?id=".$id;
