...
class GameeQuery extends Controller{
    //商家查询页面
    public function query(){

            $mydata = input('name');
            $myname = db('ae_game_name')->whereOr('name','like','%'.$mydata.'%')->whereOr('phone','like','%'.$mydata.'%')
                ->alias('a')
                ->join('ae_game_boss b','a.pid = b.id')
                ->select();
            if(!$myname){
        ...
        public function gets(){
            $myid=input('id');
            if($myid){
                $my=db('ae_game_name')->where('id',$myid)
                    ->find();
                if($my && $my['pid']==0){
                    return json_encode(array('state'=>101,'data'=>"未参加活动！"));
                }
                $myboss=db('ae_game_boss')->where('id',$my['pid'])->find();
                if($myboss && $myboss['hp']==0){
                        if($my['record']==0){
                            $my=db('ae_game_name')->where('id',$myid)->update(array('id'=>$myid,'record'=>1,'time'=>date('Y-m-d H:i:s',time())));
                            if($my) {
        ...
        public function remark(){
            ...
            $myremark=db('ae_game_name')->update(array('id'=>$myid,'remark'=>$myre));
            if($myremark){
