...
class Game extends Common {
    public function game(){
        ...
        //判断是否是分享游戏
        if($id) {
            $myuid = db('a_game_boss')->where('id', $id)->find();
            if($myuid && $myuid['hp']!=0){
                cookie('hp', $myuid['hp']);
    ...
    public function games(){
        ...
        $data=db('a_game_name')->where($myname)->find();
        if(cookie('new')==1) {
            if($data && $data['pid']!=0){
                $myboss=db('a_game_boss')->where('id',$data['pid'])->find();
                $this->assign([
            ...
            if($data){
                db('a_game_name')->update(['id'=>$data['id'],'pid'=>$id]);
                db('a_game_boss')->update(['id'=>$id,'pid'=>$data['id']]);
            }else{
                $myname['pid']=$id;
                $pid=db('a_game_name')->insertGetId($myname);
                db('a_game_boss')->update(['id'=>$id,'pid'=>$pid]);
            }
            cookie('id',$id);
            return view('mygame');
        }else{
            $myuid = db('a_game_boss')->where('id', cookie('id'))->find();
            $where = [
                'id' => cookie('id'),
                'uid' => $myuid['uid'] + 1,
            ];
    ...
    public function mygame(){
            $this->redirect('http://www.fmtoa.com/game/index.html?id='.cookie('id'));
    }
