<?php
	...
	class Oncotherapy extends Common{
...
//生成word文档
			...
			$validate=validate('Oncotherapy');
			if(!$validate->scene('add')->check($data)){
				$this->error($validate->getError());
			}
			$re=db('Oncotherapy')->insert($data);
			if($re){
	...
	public function getLevelChild($pid=0)
	{
		...
		$pids=db('Oncotherapy')->where('pid',$pid)->select();
	}
	
		
	public function lst()
	{
			$list=db('Oncotherapy')->paginate();
			$this->assign([
				'list'=>$list,
				'tabTitle'=>'肿瘤治疗列表',
			]);	
	...
	{
		$id=input('id');
		$detail_info=db('Oncotherapy')->find($id);
	...
	public function updhanddle()
	{
		...
		$re=db('Oncotherapy')->where('cancer_id',$id)->update($data);
		if($re){
