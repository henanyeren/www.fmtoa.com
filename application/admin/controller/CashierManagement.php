...
class CashierManagement extends Common
{
    public function add(){
        $re=[
            'status'=>1,
    ...
    public function addhanddle(){
        ...
        $financedata['cashier_url'] = input('cashier_url');
        $financedata['cashier_name']=input('cashier_name');
        $financedata['cashier_category']=input('cashier_category');
        $financedata['cashier_time'] = time();
        $financedata['cashier_content']=input('cashier_content');
        $validate=validate('CashierManagement');
        if(!$validate->scene('add')->check($financedata)){
            $this->error($validate->getError());
        }
        $res = db('app_cashier_management')->insert($financedata);
        if ($res) {
    ...
    public function lst(){
        $myfinance=db('app_cashier_management')->order('cashier_id desc')->select();
    ...
    {
        $id = input('cashier_id');
        $myfinance = db('app_cashier_management')->delete($id);
        if ($myfinance) {
    ...
    public function upload(){
        if(request()->isAjax()){
            $validate=\think\Loader::validate('cashier_management');
            $mypost=request()->post();
    ...
    {
        $id=input('id');
        $detail_info=db('app_cashier_management')->find($id);
    ...
    {
        $data=[
            'cashier_url' =>input('cashier_url'),
            'cashier_name'=>input('cashier_name'),
            'cashier_category'=>input('cashier_category'),
            'cashier_content'=>input('cashier_content'),
        ];


        $id=input('cashier_id');
      $validate=validate('cashier_management');
        if(!$validate->scene('add')->check($data)){
            $this->error($validate->getError());
        }
        $re=db('app_cashier_management')->where('cashier_id',$id)->update($data);
        if($re){
    ...
    public function detail()
    {
        ...
        $detail=db('app_cashier_management')->find($id);
        if($detail){
