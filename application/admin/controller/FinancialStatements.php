...
class FinancialStatements extends Common
{
    public function add(){
        $re=[
            'status'=>1,
    ...
    public function addhanddle(){
        ...
        $validate=validate('FinancialStatements');
        if(!$validate->scene('add')->check($financedata)){
            $this->error($validate->getError());
        }
        $res = db('app_financial_statements')->insert($financedata);
        if ($res) {
    ...
    public function lst(){
        $myfinance=db('app_financial_statements')->order('financial_id desc')->select();
    ...
    public function del()
    {
        ...
        $myfinance = db('app_financial_statements')->delete($id);
        if ($myfinance) {
    ...
    public function upload(){
        if(request()->isAjax()){
            $validate=\think\Loader::validate('financial_statements');
            $mypost=request()->post();
    ...
    {
        $id=input('id');
        $detail_info=db('app_financial_statements')->find($id);
    ...
    public function updhanddle()
    {
      ...
      $validate=validate('financial_statements');
        ...
        $re=db('app_financial_statements')->where('financial_id',$id)->update($data);
        if($re){
    ...
    public function detail()
    {
        ...
        $detail=db('app_financial_statements')->find($id);
        if($detail){
