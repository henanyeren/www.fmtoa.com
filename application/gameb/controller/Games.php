...
class Game extends Controller{
    ...
    public function game1(){
        $id=[
            ...
            $myuid = db('ab_game_boss')->where('id', $id['pid'])->find();
            $where = [
                'id' => $id['pid'],
                'uid' => $myuid['uid'] + 1,
            ];
            ...
            //判断信息是否填充过，有就不继续填充
            $mydata= db('ab_game_name')->where('phone',input('phone'))->select();
            if($mydata) {
    ...
    public function game2(){
        ...
        $mydata=db('ab_game_name')->where(['phone'=>$data['phone'],'pid'=>cookie('pid')])->find();
        if(cookie('hp')==0 && $mydata){
            ...
            //记录姓名和是否是新游戏
                $pid=db('ab_game_name')->insert($data);
                cookie('name', input('name'));
                cookie('new',0);
