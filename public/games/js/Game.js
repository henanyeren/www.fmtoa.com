...
var Game = /** @class */ (function () {
    function Game() {
        //子弹发射偏移位置表
        this.bulletPos = [[0], [-15, 15], [-30, 0, 30], [-45, -15, 15, 45], [-60, -30, 0, 30, 60], [-60, -40, -20, 20, 40, 60], [-60, -40, -20, 0, 20, 40, 60], [-60, -45, -30, -15, 15, 30, 45, 60],
            [-60, -45, -30, -15, 0, 15, 30, 45, 60], [-75, -60, -45, -30, -15, 15, 30, 45, 60, 75], [-75, -60, -45, -30, -15, 0, 15, 30, 45, 60, 75]];
        //关卡等级
        this.level = 0;
        //积分成绩
        this.score = 0;
        ...
        var resArray = [
            ...
            { url: "war/smbg.png", type: Laya.Loader.IMAGE }
        ];
    ...
    Game.prototype.onLoop = function () {
        ...
        //关卡越高，敌机飞行速度越快
        var speedUp = Math.floor(this.level / 7);
        //关卡越高，敌机血量越高
        var hpUp = Math.floor(this.level / 7);
        //关卡越高，敌机数量越多
        var numUp = Math.floor(this.level / 6);
        //生成小飞机
        if (Laya.timer.currFrame % (80 - cutTime) === 0) {
            this.createEnemy(0, 3 + numUp, 4 + speedUp, 1);
        }
        //生成中型飞机
        if (Laya.timer.currFrame % (150 - cutTime * 4) === 0) {
            this.createEnemy(1, 1 + numUp, 4 + speedUp, 2 + hpUp * 2);
        }
        //生成boss
        if (Laya.timer.currFrame % (900 - cutTime * 4) === 0) {
            this.createEnemy(2, 1, 1 + speedUp, 6 + hpUp * 6);
            //播放boss出场声音
            Laya.SoundManager.playSound("res/sound/enemy3_out.mp3");
        }
        var times = Laya.Browser.now();
        if (role.shootTimes < times) {
            role.shootTimes = times + 30000;
            this.createEnemy(2, 1, 0, 100);
            //播放boss出场声音
            Laya.SoundManager.playSound("res/sound/enemy3_out.mp3");
        }
    };
    Game.prototype.lostHp = function (role, lostHp) {
        ...
        if (role.heroType === 2) {
            ...
            this.hero.shootType = Math.min(Math.floor(this.bulletLevel) + 1, 10);
            //子弹级别越高，发射频率越快
            ...
            if (role.heroType > 0) {
                ...
                if (role.type === "enemy4") {
                    ...
                    //显示提示信息
                    mydatas.datas(this.HPs);
                    this.gameInfo.infoLabel.text = "win,分数：" + this.score + "\n恭喜你。";
                }
                if (role.type != "hero") {
                    //播放爆炸动画弹声音
    ...
    Game.prototype.restart = function () {
        this.HPS = mydatas.data['hp'] / 200;
        this.HPs = mydatas.data['hp'];
        //  this.HPs=mydatas.data['HP'];
        // this.HPS=mydatas.data['HP']/200;
        // console.log(mydatas.data['HP']);
        //重置游戏数据
        this.gameBoss = 0;
        this.score = 0;
        this.level = 0;
    ...
    Game.prototype.createEnemy = function (type, num, speed, hp) {
        if (speed == 0 && this.gameBoss === 0) {
            this.gameBoss = 1;
            //创建敌人
            var enemy = Laya.Pool.getItemByClass("role", Role);
            enemy.init("enemy" + (type + 2), 1, this.HPs, 0, this.radius[type + 1]);
            //随机位置
            enemy.pos(250, 100);
            //添加到舞台上
